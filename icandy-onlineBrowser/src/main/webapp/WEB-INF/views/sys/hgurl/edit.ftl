<form class="form required-validate" method="post" action="${ctx!}/hgurl/update" onsubmit="return validateCallback(this, dialogAjaxDone)">
<table class="table table-bordered"><input type="hidden" name="id" value="${item.id}">
	<tr>
		<th>接收地址</th>
		<td class="col-sm-9"><div class="input-group">
			<input class="form-control" name="hg_url" type="text" required value="${item.hgUrl}">
